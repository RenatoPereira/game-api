...
describe('MatchsService', () => {
  ...
  it ('should find a opened match and insert a new player', () => {
    const player = { id: 'player-1', name: 'Player 1' };
    const enemy = { id: 'enemy-1', name: 'Enemy 1' };
  ...
  it('should throw an error when match not found', async () => {
    ...
    await expect(service.startMatch(matchId)).rejects.toThrow(`Match (${matchId}) not found`)
  })
  ...
  it('should throw an error when match not found', async () => {
    ...
    await expect(service.cancelMatch(matchId)).rejects.toThrow(`Match (${matchId}) not found`)
  })
  ...
  it('should throw an error when match not found', async () => {
    ...
    await expect(service.finishMatch(matchId)).rejects.toThrow(`Match (${matchId}) not found`)
  })
